...
export const SPRITES: Record<string, SpriteAsset> = {
  player: {
    ...
    // First real asset shipped end-to-end (proves the placeholder→real pipeline,
    // seed §8). Generated by `npm run gen:assets`; BootScene loads it, no code change.
...
/** Count of assets backed by a real (non-placeholder) file. */
export function realAssetCount(): number {
  return (
    Object.values(SPRITES).filter((s) => s.file !== null).length +
    Object.values(AUDIO).filter((a) => a.file !== null).length
  );
}
