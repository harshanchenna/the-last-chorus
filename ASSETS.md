...
¹ `player` ships a real `public/assets/player.png` (a 1-frame Lantern-bearer, generated by
`npm run gen:assets`) — the first end-to-end proof of the placeholder→real pipeline. A full
multi-frame strip drops in by replacing the file (frame size must stay 32×32).
