...
`enemy.reliquary_warden`, `npc.wisp`, `enemy.miniboss_choirmaster` (64×64), `enemy.reliquary_echo`
(64×64). All single-frame for now; multi-frame animation strips drop in by replacing the file at the
same frame size. World-glyph placeholders (rest/lore/gate/exit/altar/refrain) and tilesets are still
programmatic.
...
| ------------------------------ | --------------------- | --------------------- |
| `zone.ashchoir.ambient`        | base, melody, tension | **real** (ElevenLabs) |
| `zone.glass_reliquary.ambient` | base, melody, tension | **real** (ElevenLabs) |
